...
class CycleDetection:
    ...
    def InsertInTable(self, table, y, i):
        table.update({y: i})
    ...
    def SearchTableY(self, table, y):
        if y in table.keys():                      
            return table[y]
        else:
            return -1

    def SearchTableJ(self, table, k):
    ...
    def fk(self, k):
        kb = self.SearchTableJ(self.table, self.b * (k // self.b))      
        if kb != -1:                                
            for n in range(k % self.b):             
                kb = self.f(kb)
        return kb

    def DetectCycle(self, x):
        y = x
        i = 0
        m = 0
        while True:
            if i % self.b == 0 and m == self.max_size:
                ...
                m = m // 2
            if i % self.b == 0:
                self.InsertInTable(self.table, y, i)
                m = m + 1
            y = self.f(y)
            i = i + 1
            if (i % (self.g * self.b)) < self.b:
                j = self.SearchTableY(self.table, y)
                if j != -1:
                    return y, i, j
    
    def RecoverCycle(self, y, i, j):
        c = 1
        found_c = False
        yc = y
...
if __name__ == "__main__":
    ...
    y, i, j = cd.DetectCycle(int(file[0]))
    c, l = cd.RecoverCycle(y, i, j)
    cd.printresults(c, l)
